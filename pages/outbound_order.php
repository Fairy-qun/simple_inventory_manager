      ...
      <div class="content">
        ...
        <div class="data">
          <table>
            ...
            <?php
            include("../pdo/pdo.php");
            // $sql = "select a.id,a.outbound_order_code,a.warehouse_code,to_date(to_char(a.outbound_time),'yyyy-MM-dd') outbound_time,a.operator,to_date(to_char(a.create_time),'yyyy-MM-dd hh24:mi:ss') create_time,a.remark,b.material_code,b.quantity from outbound_order a inner join outbound_detail b on a.outbound_order_code = b.outbound_order_code order by a.id";
            // $sql = "select a.id,a.outbound_order_code,a.warehouse_code,to_char(a.outbound_time,'yyyy-MM-dd') outbound_time,a.operator,to_char(a.create_time,'yyyy-MM-dd hh24:mi:ss') create_time,a.remark,b.material_code,b.quantity from outbound_order a inner join outbound_detail b on a.outbound_order_code = b.outbound_order_code order by a.id";

            $sql = "select a.id,a.outbound_order_code,a.warehouse_code,to_char(a.outbound_time,'yyyy-MM-dd') outbound_time,a.operator,a.create_time,a.remark,b.material_code,b.quantity from outbound_order a inner join outbound_detail b on a.outbound_order_code = b.outbound_order_code order by a.id";
            ...
            // 如果有数据
            foreach ($data as $v) {
              echo "<tr><td>" . $v['ID'] . "</td><td>" . $v['OUTBOUND_ORDER_CODE'] . "</td><td>" . $v['WAREHOUSE_CODE'] . "</td><td>" . $v['MATERIAL_CODE'] . "</td><td>" . $v['QUANTITY'] . "</td><td>" . $v['OUTBOUND_TIME'] . "</td><td>" . $v['CREATE_TIME'] . "</td><td>" . $v['OPERATOR'] . "</td><td>" . $v['REMARK'] . "</td><td style='width: 180px;'><buttom class='update' onclick='updateHandler(this)'>修改</buttom><buttom class='delete' onclick='deleteHandler(this)'>删除</buttom></td></tr>";
            }
            ?>
          </table>
        </div>
...
<script>
  ...
  const addHandler = () => {
    ...
    location.href = './add_outbound_order.php'
  }
  // 删除数据
  ...
  const comfirmHandler = () => {
    $.post('../action/deleteOutboundOrderAction.php', deleteObj, (message) => {
      console.log(message);
      if (message == 'noValue') {
        alert("必须参数不能为空")
  ...
  const updateHandler = (obj) => {
    ...
    location.href = './update_outbound_order.php'
  }
</script>
